...
// injection (injectAutoDetectOnAllTabs / onUpdated) was removed — in Firefox MV2
// chrome.scripting.executeScript returns undefined (no .catch), which threw, and
// it duplicated the now-declarative injection.
...
chrome.commands.onCommand.addListener((command) => {
  if (command === "translate-selection") {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      injectFiles(tabs[0].id, ['src/lib/tesseract.min.js', 'src/ocr_content.js'], "OCR");
    });
  } else if (command === "translate-page") {
    // Full page translation — inject content script
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      if (!tabs[0] || !tabs[0].id) return;
      injectFiles(tabs[0].id, ['src/translate_page.js'], "Page translate");
    });
  }
});

// Inject one or more script files into a tab, using whichever API the browser
// supports. Firefox MV2 has chrome.tabs.executeScript (callback, one file at a
// time); Chrome MV3 has chrome.scripting.executeScript (promise, multi-file).
function injectFiles(tabId, files, label) {
  if (chrome.tabs && chrome.tabs.executeScript) {
    const runNext = (i) => {
      if (i >= files.length) return;
      chrome.tabs.executeScript(tabId, { file: files[i] }, () => {
        if (chrome.runtime.lastError) {
          console.log(`[ShallotT] ${label} injection failed:`, chrome.runtime.lastError.message);
          return;
        }
        runNext(i + 1);
      });
    ...
    chrome.scripting.executeScript({ target: { tabId: tabId }, files: files })
      .catch(err => console.log(`[ShallotT] ${label} injection failed:`, err && err.message));
  }
}
